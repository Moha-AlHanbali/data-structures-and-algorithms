...
const toLastNames = people => {
  // Solution code here...
  let stringArray = people.map((element) => {
...
const addPurchases = (arr) => {
  // Solution code here...
  let purchases = arr.reduce((acc, val) => {
...
const returnNames = (arr) => {
  // Solution code here...
  let namesArray = arr.reduce((acc, val) => {
    acc.push(val.name) ;
    return acc;
  }, []);
  return namesArray;
...
const reversedString = (str) => {
  ...
  let reverse = strArray.reduce((acc, val) => {
    return val + acc;
  });
  return reverse;
};
...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  let children2DArray = arr.reduce((acc, val) => {
    if (typeof val.children !== 'undefined')
    {acc.push(val.children);}
    return acc;
  }, []);
  let childrenArray = children2DArray.reduce((acc, val) => {
    acc = acc.concat(val);
    return acc;
  });
...
const extractStat = (statName, arr) => {
  // Solution code here...
  let extract = arr.reduce((acc, val) => {
...
const extractChildren = (arr) => {
  // Solution code here...
  let separateArray = arr.filter((item) => {
    if(item.name.includes('a') === true){
      return true;}
  });
  let arrangeArray = separateArray.map((element) => {
    if(element.children !== undefined){
      return element.children;}
  });
  let extractArray = arrangeArray.reduce((acc, val) => {
    if(val !== undefined){
      acc = acc.concat(val);}
    return acc;
  });
