...
const screenForNames = (arr) => {
  // Solution code here...
  let result = [];
  let regexMr = /^Mr.\s([A-Za-z])/;
  let regexMrs = /^Mrs.\s([A-Za-z])/;
  let regexMS = /^Ms.\s([A-Za-z])/;
  let regexDr = /^Dr.\s([A-Za-z])/;

  arr.map((str) => {
    if (regexMr.test(str) === true || regexMrs.test(str) === true || regexMS.test(str) === true || regexDr.test(str) === true) {
      result.push(str);
    }
  })
  return result;
...
const detectTicTacToeWin = (board) => {
  ...
  let helpCheck = [row1, col1, row2, col2, row3, col3, dia1, dia2];

  let win1 = [ 'X', 'X', 'X' ];
  let win2 =  [ 'O', 'O', 'O' ];

  helpCheck.forEach((line) => {
    if (line.toString() === win1.toString() || line.toString() === win2.toString()) {
      result = true;
    }
